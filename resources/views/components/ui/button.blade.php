...
@php
    $sizeClasses = match ($size) {
        'sm' => 'px-1.5 py-1.5 text-xs font-medium rounded-' . $rounded,
...
@php
    $typeClasses = match ($type) {
        ...
        'secondary' => 'bg-white border text-gray-500 hover:text-gray-700 border-gray-200 dark:focus:ring-offset-gray-900 dark:border-gray-400/10 hover:bg-gray-50 active:bg-white dark:focus:ring-gray-700 focus:bg-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-200/60 dark:bg-gray-800/50 dark:hover:bg-gray-800/70 dark:text-gray-400 focus:shadow-outline',
...
<{!! $tagAttr !!} {!! $attributes->except(['class']) !!} class="{{ $sizeClasses }} {{ $typeClasses }} cursor-pointer inline-flex items-center justify-center {{ $widthClasses }} disabled:opacity-50 font-semibold focus:outline-none">
    {{ $slot }}
</{{ $tagClose }}>