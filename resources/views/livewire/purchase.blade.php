<div class="w-full h-full">
    ...
    <div class="flex  items-center justify-between w-full mb-4 py-4">
        ...
        <x-ui.input id="keyword" name="keyword" wire:model.live="keyword" width="sm" placeholder="Search..." />
    </div>
                ...
                <div
                    ...
                    <div class="flex relative flex-col p-6 w-full">
                        <div
                            ...
                            <h2 class="text-lg font-semibold leading-tight text-gray-800 dark:text-gray-200">Input
                                Data
                                pembelian</h2>
                        </div>
                        <p class="mb-5 text-sm text-zinc-500 dark:text-gray-400">Silahkan masukkan data pembelian
                            baru
                            dalam
                            formulir berikut untuk menambah data pembelian
                        </p>
                        <div class="flex items-center w-full">
                            <form wire:submit.prevent="storePurchase" class="space-y-6 w-full">
                                <!-- Input Component for Name -->
                                <x-ui.input label="Nama Pembelian" id="name" name="name" wire:model="name"
                                    class="w-full" />
                                <x-ui.input label="Vendor" id="vendor" name="vendor" wire:model="vendor"
                                    class="w-full" />
                                <x-ui.date-picker label="Tanggal Pembelian" id="date" name="date"
                                    wire:model="date" class="w-full" placeholder="{{ now()->format('Y-m-d') }}" />
                                <x-ui.select label="Status Pembelian" id="status" name="status" wire:model="status"
                                    placeholder="Pilih Status" :options="['baru' => 'Baru', 'bekas' => 'Bekas']" />

                                <!-- Textarea Component for Description -->
                                <x-ui.text-area label="Deskripsi" type="text" id="description" name="description"
                                    wire:model="description" class="w-full">
                                </x-ui.text-area>
                                <div class="flex gap-4">

                                    @if ($updateData == true)
                                        <x-ui.button type="primary" rounded="md" wire:click="updatePurchase()"
                ...
                <div
                    ...
                    <div class="flex relative flex-col p-4 w-full">
                        <div
                            ...
                            <h2 class="text-lg font-semibold leading-tight text-gray-800 dark:text-gray-200">Daftar
                                Data
                                pembelian </h2>
                        </div>
                        @if ($purchases->count())
                            <div class="border rounded-lg overflow-hidden dark:border-neutral-700">
                                <table class="w-full text-sm text-left text-gray-500 divide-gray-200 dark:divide-neutral-700">
                                    <thead
                                        class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-900 dark:text-gray-100">
                                        <tr class="border-b dark:border-gray-700">
                                            <th>
                                                <div class="py-3 px-6 flex items-center"> Nama </div>
                                            </th>
                                            <th>
                                                <div class="py-3 px-6 flex items-center"> Deskripsi </div>
                                            </th>
                                            <th>
                                    ...
                                    <tbody>
                                        @foreach ($purchases as $key => $value)
                                            <tr class="border-b dark:border-gray-700">
                                                <td>
                                                    <div class="py-3 px-6 flex items-center">{{ $value->name }}
                                                    </div>
                                                </td>
                                                <td>
                                                ...
                                                <td>
                                                    ...
                                                    <div class="py-3 px-6 flex  gap-2 items-center">
                                                        <x-ui.button wire:click="editPurchase({{ $value->id }})"
                                                        ...
                                                        <x-ui.button wire:click="deletePurchase({{ $value->id }})"
                        ...
                        @else
                            <p class="text-gray-500 dark:text-gray-400 text-sm">Tidak ada daftar pembelian</p>
                        @endif
                    </div>
                </div>
            </div>
